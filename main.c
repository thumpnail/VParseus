...
typedef enum {
	ebnf__TokenType__rule, // 
	ebnf__TokenType__literal, // +1
	ebnf__TokenType__assign, // +2
	ebnf__TokenType__end, // +3
	ebnf__TokenType__alt, // +4
	ebnf__TokenType__opt_open, // +5
	ebnf__TokenType__opt_close, // +6
	ebnf__TokenType__opt_repeat_open, // +7
	ebnf__TokenType__opt_repeat_close, // +8
	ebnf__TokenType__group_open, // +9
	ebnf__TokenType__group_close, // +10
	ebnf__TokenType__colon, // +11
	ebnf__TokenType__eql, // +12
}  ebnf__TokenType;
...
VV_LOCAL_SYMBOL void ebnf__lexer(void);
VV_LOCAL_SYMBOL void ebnf__parser(void);
VV_LOCAL_SYMBOL Array_string ebnf__preprocessor(Array_string lines);
VV_LOCAL_SYMBOL bool ebnf__array_contains(string c, Array_string arr);
...
ebnf__EbnfDocument ebnf__read_ebnf(string filename) {
	...
	if (_t1.is_error) {
		IError err = _t1.err;
		_v_panic(_SLIT("Could not read file..."));
		VUNREACHABLE();
	;
	}
	
 	Array_string document_lines =  (*(Array_string*)_t1.data);
	Array_string lexed = ebnf__preprocessor(document_lines);
	ebnf__EbnfDocument _t2 = ((ebnf__EbnfDocument){.rules = __new_array(0, 0, sizeof(ebnf__Rule)),});
	return _t2;
}
...
VV_LOCAL_SYMBOL Array_string ebnf__preprocessor(Array_string lines) {
	...
	Array_string words = __new_array_with_default(lines.len, lines.len * 20, sizeof(string), &(string[]){_SLIT("")});
	string word = _SLIT("");
	for (int _t1 = 0; _t1 < lines.len; ++_t1) {
		...
		if (!string__eq((*(string*)array_get(words, words.len - 1)), _SLIT("EOL"))) {
			array_push((array*)&words, _MOV((string[]){ string_clone(_SLIT("EOL")) }));
		}
	}
	array_push((array*)&words, _MOV((string[]){ string_clone((_SLIT("EOF"))) }));
	return words;
}
...
VV_LOCAL_SYMBOL void main__main(void) {
	ebnf__read_ebnf(_SLIT("script.ebnf"));
}
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	_const_ebnf__all_num = new_array_from_c_array(11, 11, sizeof(string), _MOV((string[11]){
		...
		_SLIT("9"), _SLIT(".")}));
	_const_ebnf__operator_list = new_map_init_noscan_value(&map_hash_string, &map_eq_string, &map_clone_string, &map_free_string, 11, sizeof(string), sizeof(ebnf__TokenType),
		_MOV((string[11]){
			_SLIT(":="), 
			_SLIT(";"), 
			_SLIT("|"), 
...
;
	_const_ebnf__symbol_list = new_map_init_noscan_value(&map_hash_string, &map_eq_string, &map_clone_string, &map_free_string, 12, sizeof(string), sizeof(ebnf__TokenType),
		_MOV((string[12]){
			_SLIT(":"), 
			_SLIT("="), 
			_SLIT(";"), 
